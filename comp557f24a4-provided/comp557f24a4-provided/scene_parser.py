...
def load_scene(infile: str):
    ...
    if "dof" in data["camera"] and data["camera"]["dof"] == True: # Get & set DOF settings
        dof = True
        focal_length = data["camera"]["focal_length"]
        aperture = data["camera"]["aperture"]
        dof_samples = data["camera"]["dof_samples"]
    else: # Or set default DOF settings
        dof = False
        focal_length = 1
        aperture = 0
        dof_samples = 1
    ...
    for light in data.get("lights", []):
        ...
        if l_type == "point":
            ...
            l_vector = make_vec3(light["position"])
            l_attenuation = glm.vec3(1,0,0) if "attenuation" not in light else make_vec3( light["attenuation"] )
            l_direction = glm.normalize( make_vec3(light["direction"]) )
            l_radius = light["radius"]
            l_samples = light["samples"]
        else:
            print("Unkown light type", l_type, ", skipping initialization")
            continue
        lights.append(hc.Light(l_type, l_name, l_colour * l_power, l_vector, l_attenuation, l_direction, l_radius, l_samples))
    ...
    return scene.Scene(width, height, jitter, samples,  # General settings
                dof, focal_length, aperture, dof_samples,  # Depth of field settings
...
def load_geometry( geometry, material_by_name, geometry_by_name ):
    ...
    if g_type == "sphere":
        ...
        g_s = make_vec3(geometry.get("scale", [1, 1, 1]))
        M = make_matrix(g_pos, g_r, g_s)
        left = load_geometry(geometry["pair"][0], material_by_name, geometry_by_name)
        geometry_by_name[left.name] = left
        right = load_geometry(geometry["pair"][1], material_by_name, geometry_by_name)
        geometry_by_name[right.name] = right
        node = geom.CSG(g_name, g_type, left, right, geometry["operation"], M, g_mats)
        return node
    else:
